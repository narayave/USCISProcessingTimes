...
class CosmosOps:
    def __init__(self, container_name):
        ...
        host = 'https://uscis-proc-cosmos-db.documents.azure.com:443/'
    ...
    def _db_setup(self):
        ...
        try:
            try:
                self.db_client = self.cosmos_client.create_database(
                    id=self.cosmos_database)
                print(f"Database with id '{self.cosmos_database}' created")

            except exceptions.CosmosResourceExistsError:
                self.db_client = self.cosmos_client.get_database_client(
            ...
            try:
                ...
                self.container_client = self.db_client.get_container_client(
...
class ProcessingTimes:
    ...
    def upload_submission_restapi_url(self, option_combo, request):
        ...
        option_combo["gather_date"] = datetime.datetime.now().strftime(
            '%Y-%m-%mT%H:%M:%S')
    ...
    def get_processing_time(self, option_combo):
        ...
        self.select_dropdown_value(
            self.category_select, option_combo["categoryKey"])
        self.select_dropdown_value(
            self.office_select, option_combo["officeKey"])
